...
- 사용자가 "현재 변경 사항 커밋 메시지 작성해줘"처럼 메시지만 요청하면 `git diff`와 `git status`로 변경 내용을 확인한 뒤 메시지만 제안한다.
- 사용자가 "커밋해줘"처럼 실제 커밋을 명시한 경우에만 stage/commit을 수행한다.
...
7. GitHub 이슈를 생성한다.
8. 사용자가 브랜치 생성까지 요청하지 않았다면 여기서 멈추고 생성된 이슈 번호와 다음 브랜치 후보만 안내한다.
9. 사용자가 브랜치 생성까지 요청했거나 바로 작업할 서브 이슈를 명시한 경우에만 브랜치를 생성한다.
10. 브랜치를 생성할 때는 생성된 이슈 번호를 사용해 브랜치명을 정한다. 여러 이슈가 있으면 실제로 작업할 서브 이슈 번호를 브랜치명에 사용한다.
11. 깨끗한 최신 `main`에서 브랜치를 생성한다.
12. 브랜치명, 연결된 메인/서브 이슈, 로그 검증 방향을 사용자에게 알려준다.
...
7. 사용자가 메시지만 요청했다면 메시지만 제안하고 멈춘다.
8. 사용자가 커밋까지 요청했다면 변경 파일을 stage하고 커밋한다.
...
4. `git status --short`로 미커밋 변경이 남아 있는지 확인한다.
5. 미커밋 변경이 있으면 push 전에 멈추고, 사용자가 해당 변경을 커밋할지 또는 제외할지 확인한다.
6. 필요하면 빌드/검증 결과를 확인하되, 사용자가 직접 작업한 변경을 임의로 고치지 않는다.
7. 브랜치를 push한다.
8. PR이 없을 때만 PR 템플릿에 맞춰 새 PR 본문을 작성한다.
9. PR assignee를 `jaeunda`로 설정한다.
10. PR에는 연결된 이슈에 맞는 label을 붙인다.
11. `Etc`에는 `Verification Criteria`와 `Verification Evidence`를 구분해서 적는다.
12. 실제 스크린샷 파일이나 이미지 첨부는 사용자가 직접 반영할 수 있으므로, 에이전트는 `Verification Criteria`에 검증 완료 조건을 명확히 남긴다.
13. 펌웨어 동작 변경이 아니면 `Verification Evidence`에 not applicable 사유를 남긴다.
14. 펌웨어 동작 변경인데 스크린샷이 아직 없으면 `Verification Evidence`에 첨부 예정 메모를 남긴다.
15. PR URL과 요약, 검증 여부를 사용자에게 알려준다.
